...
from ..layers.mixed_precision import fp16_sr_bit, round_complex
            ...
            for p in group["params"]:
                ...
                if grad.dtype == torch.complex32:
                    ...
                    if torch.is_complex(grad):
                        # Apply stochastic rounding for complex32 computations
                        if hasattr(torch, 'complex'):
                            grad_abs_sq = (grad * grad.conj()) * (1.0 - beta2)
                            # Apply stochastic rounding to real and imaginary parts separately
                            if grad_abs_sq.dtype == torch.complex32:
                                grad_abs_sq_real = stochastic_round_fp16(grad_abs_sq.real)
                                grad_abs_sq_imag = stochastic_round_fp16(grad_abs_sq.imag)
                                grad_abs_sq = torch.complex(grad_abs_sq_real, grad_abs_sq_imag)
                            exp_avg_sq.add_(grad_abs_sq)
                        else:
                            exp_avg_sq.add_((grad * grad.conj()) * (1.0 - beta2))
                    else:
                        grad_sq = (grad * grad) * (1.0 - beta2)
                        # Apply stochastic rounding for half precision
                        if grad.dtype == torch.float16 or grad.dtype == torch.complex32:
                            grad_sq = stochastic_round_fp16(grad_sq)
                        exp_avg_sq.add_(grad_sq)
                else:
                ...
                # Apply stochastic rounding for half-precision parameters
                if p.dtype == torch.float16 or (torch.is_complex(p) and p.dtype == torch.complex32):
                    if torch.is_complex(norm_grad):
                        norm_grad = round_complex(norm_grad, precision="fp16")
                    else:
                        norm_grad = fp16_sr_bit(norm_grad)
                
                # GaLore Projection Back
                if group.get('galore', False):
                    norm_grad = state["projector"].project_back(norm_grad)
                    # Apply stochastic rounding after projection if using half precision
                    if p.dtype == torch.float16 or (torch.is_complex(p) and p.dtype == torch.complex32):
                        if torch.is_complex(norm_grad):
                            norm_grad = round_complex(norm_grad, precision="fp16")
                        else:
                ...
                # For half-precision parameters, use stochastic rounding when applying the update
                if p.dtype == torch.float16 or (torch.is_complex(p) and p.dtype == torch.complex32):
                    # Calculate update
                    update = norm_grad * (-step_size)
                    if torch.is_complex(update):
                        update_real = stochastic_round_fp16(update.real)
                        update_imag = stochastic_round_fp16(update.imag)
                        update = torch.complex(update_real, update_imag)
                    else:
                        update = stochastic_round_fp16(update)
                    p.add_(update)
                else:
                    # For full precision, use the original update
                ...
                if group["weight_decay"] > 0.0:
                    weight_decay_term = p * (-group["lr"] * group["weight_decay"])
                    # Apply stochastic rounding for half-precision parameters
                    if p.dtype == torch.float16 or (torch.is_complex(p) and p.dtype == torch.complex32):
                        if torch.is_complex(weight_decay_term):
                            weight_decay_real = stochastic_round_fp16(weight_decay_term.real)
                            weight_decay_imag = stochastic_round_fp16(weight_decay_term.imag)
                            weight_decay_term = torch.complex(weight_decay_real, weight_decay_imag)
                        else:
                            weight_decay_term = stochastic_round_fp16(weight_decay_term)
                    p.add_(weight_decay_term)
